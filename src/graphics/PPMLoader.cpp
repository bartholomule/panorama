...
#include "panorama/graphics/PPMLoader.hpp"
	...
	namespace
	{
		...
		*/
		...
		bool readBinaryColor(const char*& data, unsigned& len, GenericRGBColor<unsigned char>& color, unsigned maxColor)
		{
			...
			unsigned r = 255 * readBinaryColorEntry(data, maxColor) / maxColor;
			unsigned g = 255 * readBinaryColorEntry(data, maxColor) / maxColor;
			unsigned b = 255 * readBinaryColorEntry(data, maxColor) / maxColor;
		...
		bool loadP3(const char* data, unsigned len, GenericImage<GenericRGBColor<unsigned char> >& image)
		{
		...
		bool loadP6(const char* data, unsigned len, GenericImage<GenericRGBColor<unsigned char> >& image)
		{
			unsigned width, height, maxColor;
			if( !readHeader(data, len, width, height, maxColor) )
			{
