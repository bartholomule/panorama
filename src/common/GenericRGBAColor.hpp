...
namespace panorama
{
	...
	blocxx::String GenericRGBAColor<T, ColorTraits>::toString(const Indentation& indent, StringDumpable::PrefixType prefix) const
	{
		...
		return indent.initial() + tag +
			"[- " +
			panorama::toString(r(), indent.indentInside(), prefix) +
			"," +
			panorama::toString(g(), indent.indentInside(), prefix) +
			"," +
			panorama::toString(b(), indent.indentInside(), prefix) +
			"," +
			panorama::toString(a(), indent.indentInside(), prefix) +
			" -]";
	}
...
#endif /* !defined(PANORAMA__GENERIC_RGBACOLOR_HPP) */
