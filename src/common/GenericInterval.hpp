...
#if       !defined(PANORAMA_INTERVAL_HPP)
#define            PANORAMA_INTERVAL_HPP
...
namespace panorama
{
	...
	blocxx::String GenericInterval<T>::toString(const Indentation& indent, StringDumpable::PrefixType prefix) const
	{
		...
		return indent.initial() + tag +
			"[" +
			panorama::toString(begin(), indent.indentInside(), prefix) +
			"," +
			panorama::toString(end(), indent.indentInside(), prefix) +
			"]";
	}

} // namespace panorama
#endif /* !defined(PANORAMA_INTERVAL_HPP) */
