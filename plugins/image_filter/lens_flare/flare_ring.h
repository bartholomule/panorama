...
#ifndef _FLARE_RING__
#define _FLARE_RING__

#include "flare.h"

class TFlare_Ring : public TFlare
{
  ...
  protected:
    ...
    float calcPixelIntensity (float DX, float DY)
    {
      ...
      if ( _fPixelIntensity > 0.0 ) 
      {
        ...
        {
          _fPixelIntensity = 1.0 - ((_fPixelIntensity - 1.0) * fParameter);
        }
      }
      else 
      {
  ...
  public:
    ...
    TFlare_Ring (TColor tCOLOR, ELFForms eFORM, float fSIZE, float fTRANSLATION, float fPARAMETER, TImage* ptIMAGE) : 
      TFlare (tCOLOR, eFORM, fSIZE, fTRANSLATION, ptIMAGE),
      fParameter (fPARAMETER) {}

    TFlare_Ring (TGradient* ptGRADIENT, ELFForms eFORM, float fSIZE, float fTRANSLATION, float fPARAMETER, TImage *ptIMAGE) : 
