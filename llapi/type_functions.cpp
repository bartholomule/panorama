...
namespace panorama
{
  DEFINE_EXCEPTION(TypeConversionException);

  std::string EAttribType_to_str (EAttribType eTYPE)
  {

    /**
     * This function is pretty dumb, but I (KH) couldn't find another one
     * anywhere else.  Added on 07Aug2000 to the parser, moved here on
     * 01Sep2001, while attempting to further expand the abilities of panorama.
     */
  ...
  EAttribType str_to_EAttribType (const std::string& s)
  {
    if( s == "none" )
    {
      return FX_NONE;
    ...
    const magic_pointer<TAttribute>& a2)
  {
    if( !a1 || !a2 )
    {
      GOM.error() << "One of the args is NULL!" << std::endl;
      if( !a1 )
        GOM.error() << "it is a1! [ the required parameter ]" << std::endl;
      else
        GOM.error() << "it is a2! [ the coerceable argument passed in ]" << std::endl;
      return false;
    }
    //  GOM.debug() << "types_match: comparing " << a1->AttributeName()
    //       << " and " << a2->AttributeName() << std::endl;
